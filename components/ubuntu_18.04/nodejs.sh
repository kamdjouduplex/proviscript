...
func::proviscript_msg info "Checking if Node.js is installed, if not, proceed to install it."
...
if [ "${is_nodejs_installed}" == "install ok installed" ]; then
    ...
    func::proviscript_msg info "Try \"sudo ${_PM} purge nodejs\""
    exit 2
fi
...
# Install Node.js
func::proviscript_msg info "Proceeding to install Node.js."
...
if [[ "${nodejs_version}" = "v"* && "${nodejs_version}" != *"command not found"* ]]; then
    ...
    func::proviscript_msg success "$(nodejs -v 2>&1)"
else
